    ...
    new Promise((resolve, reject) => {
        ...
        steemStream.api.streamOperations((error, operation) => {
            ...
            if(operation && operation[0] === 'comment') {
                if(operation[1].parent_author === '%') {
                    if(!blacklist.includes(operation[1].author)) {
                        try {
                            ...
                            // Removing the punctuation at the end of some mentions, lower casing mentions and removing duplicates
                            let mentions = _.uniq(metadata.users.map(user => user.replace(/[?¿!¡.,;:-]+$/, '').toLowerCase()));
                ...
                } else if(operation[1].parent_author === 'checky') {
